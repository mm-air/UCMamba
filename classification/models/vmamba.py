...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        if forward_type in ('v05h', 'v05v', 'v05s'):
            self.k_group = 2
        elif forward_type in ('v05hv', 'v05hs', 'v05sv'):
            self.k_group = 4
        elif forward_type == 'v05hvs':
            self.k_group = 6
        elif forward_type in ('v05s1d', 'v05h1d', 'v05v1d'):
            self.k_group  = 1
        ...
        FORWARD_TYPES = dict(
            ...
            # ============================= by Shirley
            v05hvs= partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="hvs"),
            v05hv = partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="hv"),
            v05hs=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="hs"),
            v05sv=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="sv"),
            v05h=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="h"),
            v05v=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="v"),
            v05s=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="s"),
            v05s1d=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="s1d"),
            v05h1d=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="h1d"),
            v05v1d=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode="v1d"),
        )
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        _scan_mode = dict(cross2d=1, cascade2d=-1, hvs=0, hv=1, hs=2, sv=3, s=4, h=5, v=6, s1d=7, h1d=8, v1d=9).get(scan_mode, None) if isinstance(scan_mode,  str) else scan_mode  # by Shirley
        delta_softplus = True
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.add_ctr_branch = add_ctr_branch
        if add_ctr_branch:
            self.ctr_branch = nn.Sequential(OrderedDict(
                norm=norm_layer(self.num_features),  # B,H,W,C
                permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
                avgpool=nn.AdaptiveAvgPool2d(1),
                flatten=nn.Flatten(1),
                fc1=nn.Linear(self.num_features, 512),
                relu=nn.ReLU(),
                fac2=nn.Linear(512, 128),
            ))
    ...
    def forward(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            x = layer(x)
        if self.add_ctr_branch:
            pred = self.classifier(x)
            fea_ctr = self.ctr_branch(x)
            return pred, fea_ctr
        else:
            x = self.classifier(x)
            return x
