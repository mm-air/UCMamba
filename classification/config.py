...
_C.MODEL.VSSM.USE_CTR_BRANCH = True
...
_C.TRAIN.BASE_LR = 5e-4 #5e-4
_C.TRAIN.WARMUP_LR = 5e-7
_C.TRAIN.MIN_LR = 5e-6
# Clip gradient norm
_C.TRAIN.CLIP_GRAD = 5.0
# Auto resume from latest checkpoint
_C.TRAIN.AUTO_RESUME = True #True
...
def update_config(config, args):
    ...
    # Add by Shirley
    if _check_args('save_folder'):
        config.MODEL.NAME = args.save_folder
    if _check_args('epochs'):
        config.TRAIN.EPOCHS = args.epochs
    if _check_args('lr'):
        config.TRAIN.BASE_LR = args.lr
    if hasattr(args, 'use_ctr'):
        config.MODEL.VSSM.USE_CTR_BRANCH = args.use_ctr
    if _check_args('ssm_forwardtype'):
        config.MODEL.VSSM.SSM_FORWARDTYPE = args.ssm_forwardtype
